...
! $1 $2 $4 $3 $5 $6 $7 $8 $9
	...
	ifelse($6, 1, {
		...
		ld	[out2+284], local5     ! 0xFF used in the rounds
		or	local2, local1, $3
		xor	$4, out0, local1
...
$4:
	...
	ldub	[out2+local1], local1     ! 7 (and 0xFF)
	srl	out1, 24, local0          ! 8
	and	out1, local5, local4      ! 4

	ldub	[out2+local0], local0     ! 8 (and 0xFF)
...
! next round
	...
	ldub	[out2+local3], local3     ! 7 (and 0xFF)
...
! $1 $2 $3 $4 $5 $6 $7 $8 $9
	...
	srl	$2, 3, $2
	sethi	%hi(0xFF), local2

	or	$2, local1, $2
	or	local2, %lo(0xFF), local2

	srl	$2, 1, local3
	sethi	%hi(0xFF), local1
	xor	local3, $1, local3
	or	local1, %lo(0xFF), local1
	and	local3, local2, local3
	sethi	%hi(0xFF), local4
	sll	local3, 1, local2
...
! This table is used for AND 0xFF when it is known that register
